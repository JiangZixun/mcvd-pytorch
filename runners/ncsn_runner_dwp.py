...
def make_viz_img(f, rgb_bands=None):
    """
    f: np.ndarray
       可能是 (C,H,W) 或 (H,W,C) 或 (H,W)
    rgb_bands: tuple/list 长度为3，指定从多光谱里挑哪三个通道做伪彩
               例如 (0,1,4)；不传则用单通道灰度
    返回: (H, W, 3) uint8
    """
    arr = f
    ...
    if arr.ndim == 3:
        ...
        if arr.shape[0] in (1, 3, 4, 8):  # (C,H,W)
    ...
    if C > 4:
        if rgb_bands is not None and len(rgb_bands) == 3:
            # 伪彩：挑3个谱段做 RGB（注意 OpenCV 用 BGR，但这里仅作标注，不影响）
            r = _to_uint8(arr[..., rgb_bands[0]])
            g = _to_uint8(arr[..., rgb_bands[1]])
            b = _to_uint8(arr[..., rgb_bands[2]])
            vis = np.stack([r, g, b], axis=-1)
        else:
            # 默认：取某一个通道做灰度（比如 10.4µm 或 11.2µm），这里示例取第0通道
            g = _to_uint8(arr[..., 0])
            vis = np.stack([g, g, g], axis=-1)
    elif C == 1:
        g = _to_uint8(arr[..., 0])
        ...
        vis = _to_uint8(arr[..., :3])  # 取前三个通道作为可视化
    else:
        # 兜底：取第0通道做灰度
        g = _to_uint8(arr[..., 0])
        vis = np.stack([g, g, g], axis=-1)
    ...
    def __init__(self, args, config, config_uncond):
        ...
        if config.wandb.use:
            run = wandb.init(
                ...
                config=OmegaConf.to_container(config, resolve=True) if hasattr(config, "keys") else config,
            )
                ...
                if (step % self.config.training.snapshot_freq == 0 or step % self.config.training.sample_freq == 0) and self.config.training.snapshot_sampling:
                    ...
                    if conditional:
                        ...
                        for t in range(condi.shape[1]//self.config.data.channels):
                            ...
                            frame = frame.permute(0, 2, 3, 1).numpy()[..., :1]
                        ...
                        if self.config.data.channels == 1 or self.config.data.dataset == "HIMAWARI":
                            data = torch.cat([condi[:, :1, ...], padding[:, :1, ...], reali[:, :1, ...], padding[:, :1, ...], pred[:, :1, ...]], dim=-1)
                            if future > 0:
                        ...
                        # Stretch out multiple frames horizontally
                        pred = stretch_image(pred, self.config.data.channels, self.config.data.image_size)[:, 0:1, ...]
        ...
        for i, (real_, _) in tqdm(enumerate(dataloader), total=min(max_data_iter, len(dataloader)), desc="\nvideo_gen dataloader"):
            ...
            if real.shape[1] < pred.shape[1]: # We cannot calculate MSE, PSNR, SSIM
                ...
                for ii in range(len(pred)):
                    ...
                    for jj in range(num_frames_pred):
                        ...
                        pred_ij_pil = Transforms.ToPILImage()(pred_ij[:1]).convert("RGB")
                        real_ij_pil = Transforms.ToPILImage()(real_ij[:1]).convert("RGB")
            ...
            if i == 0 or preds_per_test == 1: # Save first mini-batch or save them all
                ...
                for t in range(cond.shape[1]//self.config.data.channels):
                    ...
                    frame = torch.cat([cond_t, 0.5*torch.ones(*cond_t.shape[:-1], 2), cond_t], dim=-1)
                    frame = frame.permute(0, 2, 3, 1)[..., :1].numpy()
                    frame = np.stack([putText(f.copy(), f"{t+1:2d}p", (4, 15), 0, 0.5, (1,1,1), 1) for f in frame])
                ...
                def save_pred(pred, real, band=0):
                    if train:
                    ...
                    # B, C, H, W*T
                    cond_image = stretch_image(cond, self.config.data.channels, self.config.data.image_size)
                    pred_image = stretch_image(pred, self.config.data.channels, self.config.data.image_size)
                    real_image = stretch_image(real, self.config.data.channels, self.config.data.image_size)

                    for band in range(self.config.data.channels): 
                        cond_im = cond_image[:, band:band+1, ...]
                        pred_im = pred_image[:, band:band+1, ...]
                        real_im = real_image[:, band:band+1, ...]
                        ...
                        image_grid = make_grid(data, nrow=nrow, padding=6, pad_value=0.5)
                        if train:
                            os.makedirs(os.path.join(self.args.log_sample_path, f"videos_stretch_pred_{ckpt}_{i}"), exist_ok=True)
                            save_image(image_grid, os.path.join(self.args.log_sample_path, f"videos_stretch_pred_{ckpt}_{i}", f"band_{band}.png"))
                        else:
                            os.makedirs(os.path.join(self.args.video_folder, f"videos_stretch_pred_{ckpt}_{i}"), exist_ok=True)
                            save_image(image_grid, os.path.join(self.args.video_folder, f"videos_stretch_pred_{ckpt}_{i}", f"band_{band}.png"))
