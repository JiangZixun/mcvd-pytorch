...
class LightweightUNet(nn.Module):
    ...
    def __init__(self, frame_channels, in_frames, out_frames, base_channels=32):
        ...
        # 输出层：预测t_out个时间步的速度场
        self.velocity_head = nn.Conv2d(base_channels, out_frames * 2, 1)  # t_out个时间步 × 2个速度分量
...
class SimVPUNet(nn.Module):
    def __init__(self,shape_in, hid_S=16, hid_T=256, N_S=4, N_T=8, incep_ker=[3,5,7,11], groups=8):
        super().__init__()
        T,C,H,W = shape_in
        self.out_frames = T
        self.unet = SimVP(shape_in, hid_S, hid_T, N_S, N_T, incep_ker, groups)
        self.velocity_head = nn.Conv2d(T*C, T * 2, 1)  # t_out个时间步 × 2个速度分量

    def forward(self, x):
        x = self.unet(x)
        # 输出速度场
        x = rearrange(x, 'b t c h w -> b (t c) h w')
        velocity_fields = self.velocity_head(x)  # [B, 12, H, W]
        # 重塑为 [B, 6, H, W, 2]
        B, _, H, W = velocity_fields.shape
        velocity_fields = velocity_fields.view(B, self.out_frames, 2, H, W).permute(0, 1, 3, 4, 2)
